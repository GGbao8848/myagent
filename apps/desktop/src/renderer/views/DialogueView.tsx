...
import { api, startLocalChat, stopLocalChat } from "../api";
...
export default function DialogueView({ activeSessionId, onSelectSession }: Props) {
  ...
  const [securityMode, setSecurityMode] = useState<"auto" | "dangerous_confirm" | "always_confirm">(
    "dangerous_confirm"
  );
  ...
  const deleteSession = async (id: string) => {
    ...
    // 中止正在运行的本地 agent（防止删除后任务继续空跑）
    stopLocalChat(id).catch(() => {});
  ...
  const send = async (sessionId: string, content: string) => {
    ...
    // 本地 agent：订阅主进程事件流，直到 done/error 结束（agent 在主进程跑）
    await new Promise<void>((resolve) => {
      const unsubscribe = window.electronAPI!.onAgentEvent((data: any) => {
        if (data?.sessionId !== sessionId) return;
        const h = eventHandlers[data.event];
        if (h) h(data);
      ...
      window.electronAPI!.chat(sessionId, content, securityMode).catch(() => {
        unsubscribe();
        finish();
        resolve();
      });
          ...
          {state.streaming ? (
            ...
            <div className="space-y-1.5">
              <div className="flex items-center gap-1 text-xs">
                <span className="text-muted-foreground mr-1">安全模式：</span>
                {[
                  { key: "auto", label: "完全自动" },
                  { key: "dangerous_confirm", label: "危险操作确认" },
                  { key: "always_confirm", label: "每次确认" },
                ].map((m) => (
                  <button
                    key={m.key}
                    onClick={() => setSecurityMode(m.key as typeof securityMode)}
